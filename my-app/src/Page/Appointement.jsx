...
const Appointement = () => {
  ...
  const { doctors,currencySmbol} = useContext(AppContext);
...
//getting currennt date
  ...
  for(let i=0;i<7;i++)
  {
   ...
   //setting end tiem of date with index
   let endTime = new Date(currentDate); // ✅ base on currentDate
   ...
   while(currentDate<endTime)
   {
    ...
    //increment current time by 3o min
  ...
  return (
    <div>
      {/* Only render when docInfo is available */}
      {docInfo ? (
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
       ...
       <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4 '>
        {docSlot.length && docSlot[slotIndex].map((item,index)=>(
         <p onClick={()=>setslotTime(item.time)} key={index} className={`text-sm font-light flex-shrink-0 px-5 border py-2 rounded-full cursor-pointer ${item.time === slotTime ? "bg-primary text-white ":"text-gray-400 border border-gray-300"}`}>
